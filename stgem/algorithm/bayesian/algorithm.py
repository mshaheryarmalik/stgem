...
class BayesOptSampler(Algorithm):
    ...
    def getVector(self, feedback=None):
        ...
        if len(self.X) < self.init_num:
            ...
            sample = np.asarray(BO.suggest_next_locations()[0]) # BO.suggest_next_locations()[0]
    ...
    def do_generate_next_test(self, active_outputs, test_repository, budget_remaining):
        if self.rounds > 0:
            x, z, y = test_repository.get(-1)  # Robustness generated by SUT needed to update algorithm
            rho = y
