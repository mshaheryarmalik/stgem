...
class simulated_annealing(Algorithm):
    ...
    def do_generate_next_test(self, active_outputs, test_repository, budget_remaining):
        ...
        def neighbor(x):
            ...
            neighbor = x + radius * np.random.uniform(-1, 1, size=self.search_space.input_dimension)
            np.clip(neighbor, -1, 1, out=neighbor)
            return neighbor

        radius = 0.01 #precision 
        dim = self.search_space.input_dimension #input dimension
        rounds = 0 #number of loop
        invalid = 0 #number of invalid test
        T = 100*budget_remaining**8 #the temperature which is between 0 and 100 
        
        #====Random for the 2 first iterations====
        if (rounds<2):
            test = np.random.uniform(-1, 1, size=self.search_space.input_dimension)
            new_test = test
            rounds += 1

        while True and (rounds >= 2):
            rounds += 1
            ...
            if (f_Prev > f_Next): 
                x,z,y =test_repository.get() #values of the test that we get before the previous one (wich is called the "prev")
